...
export function FriendLibrary({
  ...
  const [chosen, setActive] = useState<SectionSlug | null>(null);
  const [openId, setOpenId] = useState<string | null>(null);

  // items arrives as [] on the first render (the parent fetches it async),
  // so derive the tab rather than fixing it in state: whatever was tapped
  // while it has anything, otherwise the first section that does.
  const active: SectionSlug =
    chosen && available.includes(chosen) ? chosen : (available[0] ?? "games");
